...
breadth
  ...
  fitted_datasets <- fitted_datasets %>%
    left_join(breadth_summary %>% select(curve_ID, model, tmin_breadth, tmax_breadth, my_breadth), join_by(curve_ID, model))
  ...
  line_data_freshwater <- fitted_datasets %>%
    filter(land_or_sea == "freshwater", breadth_TF == TRUE) %>%
    select(curve_ID, latitude, tmin_breadth, tmax_breadth)
  ...
  ggplot(freshwater_coarse, aes(x = latitude)) +
    ...
    line_data_marine <- fitted_datasets %>%
      filter(land_or_sea == "marine", breadth_TF == TRUE) %>%
      select(curve_ID, latitude, tmin_breadth, tmax_breadth)
   ...
   marine <- ggplot(fits_with_temps %>% filter(breadth_TF == TRUE) %>% filter(enviornment == "marine"), aes(x = latitude)) +
   ...
   freshwater <- ggplot(fits_with_temps %>% filter(breadth_TF == TRUE) %>% filter(enviornment == "freshwater"), aes(x = latitude)) +
